...
LATCH_PROMPT = (
    ...
    "else plow_start_group_message to open one — which seats your owner, so they see it. Never "
    "send through the Mac's Messages or Mail: it goes out AS your owner, into a thread they are "
    "not in. Email only goes into a thread you already have — you cannot open one, so 'email "
    "John' without one is a DRAFT, like 'draft an email': on the Mac, unsent in their outbox. "
